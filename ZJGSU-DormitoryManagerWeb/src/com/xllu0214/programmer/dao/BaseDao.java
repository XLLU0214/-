...
import com.xllu0214.programmer.util.DbUtil;
...
public class BaseDao<T> {
	...
	private String buildSql(int type) {
		...
		case CURD_UPDATE:{
			...
			sql = sql.substring(0,sql.length()-1) + " where id = ?";//length()-1去掉最后一个
			break;
		}
		case CURD_DELETE:{
	...
	public void closeConnection(){
		if(con != null){
			try {
				con.close();
				System.out.println(StringUtil.convertToUnderLine(t.getSimpleName()) + "Dao建立的数据库链接成功关闭！");
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
